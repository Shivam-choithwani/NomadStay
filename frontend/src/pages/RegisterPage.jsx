...
export default function RegisterPage() {
  ...
  async function onSubmit(formData) {
    setServerError("");
    try {
      const { accessToken, user } = await registerUser(formData);
      ...
      <section className="flex-grow md:w-1/2 flex flex-col items-center justify-center relative -mt-10 md:mt-0 z-10">
        <div className="w-full h-full bg-surface-container-lowest md:bg-transparent rounded-t-[40px] md:rounded-none px-margin-mobile md:px-margin-desktop py-stack-lg flex flex-col max-w-[500px] mx-auto shadow-[0px_-8px_40px_rgba(0,0,0,0.08)] md:shadow-none">
          ...
          <div className="flex items-center justify-center mb-stack-lg p-1 bg-surface-container rounded-full w-full max-w-[320px] mx-auto">
            <Link to="/login" className="flex-1 text-center py-3 px-6 rounded-full font-label-md text-label-md text-on-surface-variant hover:text-on-surface transition-colors">Sign In</Link>
            <Link to="/register" className="flex-1 text-center py-3 px-6 rounded-full font-label-md text-label-md bg-surface-container-lowest text-primary shadow-sm transition-all">Register</Link>
          </div>
          ...
          <div className="text-center md:text-left mb-stack-lg">
            ...
            <p className="font-body-md text-body-md text-on-surface-variant">Join a global community of hosts and travelers.</p>
          </div>
          ...
          <form className="space-y-stack-lg w-full" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex flex-col gap-2">
              <label className="font-label-md text-label-md text-on-surface-variant ml-1 uppercase" htmlFor="name">Full Name</label>
              ...
              {errors.name && <p className="text-sm mt-1.5 text-error">{errors.name.message}</p>}
            </div>
            ...
            {/* Password Field */}
            <div className="flex flex-col gap-2">
              <label className="font-label-md text-label-md text-on-surface-variant ml-1 uppercase" htmlFor="password">Password</label>
              <div className="relative flex items-center focus-within:shadow-[0_0_0_4px_rgba(0,92,187,0.1)] rounded-xl border border-outline-variant bg-surface-container-lowest transition-all group focus-within:border-primary">
            ...
            >
              ...
              {!isSubmitting && <span className="material-symbols-outlined text-[20px]">person_add</span>}
            </button>
          </form>
