...
export default function CreateListingPage() {
  ...
  if (user && user.idVerificationStatus !== "approved") {
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center">
        <span className="material-symbols-outlined text-[64px] text-primary mb-4" style={{ fontVariationSettings: "'FILL' 1" }}>policy</span>
        <h1 className="text-2xl font-semibold text-gray-900 mb-2">Verification Required</h1>
        <p className="text-gray-600 mb-6">
          To maintain a safe and trusted community, we require all hosts to verify their Government ID before creating a listing.
        </p>
        <Link 
          to="/profile/verify" 
          className="inline-block bg-primary text-on-primary px-6 py-3 rounded-xl font-label-md text-label-md shadow-md hover:opacity-90 transition-all"
        >
          Verify ID Now
        </Link>
      </div>
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-semibold text-gray-900 mb-6">Host a stay</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <input
            {...register("amenities")}
            placeholder="Wifi, Private room, Kitchen access"
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            rows={3}
            {...register("houseRules")}
            placeholder="e.g. No smoking inside, quiet hours after 10 PM, respect shared areas"
